...
- **Global scope**: `/Users/apaderin/dev/octoconv/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/apaderin/dev/octoconv/.claude/notes/`)
...
2. Glob `/Users/apaderin/dev/octoconv/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/apaderin/dev/octoconv/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/apaderin/dev/octoconv/.claude/notes/` — works in any directory
