...
- [x] **WEBHOOK-06**: Оператор может отключить SSRF-блокировку RFC1918 приватных адресов (`WEBHOOK_ALLOW_PRIVATE_IPS`) для деплоев на внутренней сети компании, где `callback_url` легитимно указывает на приватный IP; loopback, link-local (включая cloud metadata endpoint) и unspecified остаются заблокированы всегда, независимо от флага

### Reconciler

- [x] **RECON-04**: Reconciler находит задачи в статусе `done`/`failed` с непустым `callback_url`, для которых нет ни одной записи в `webhook_deliveries`, и инициирует доставку вебхука (закрывает гонку потери вебхука при сбое Redis в момент завершения задачи)
- [x] **RECON-05**: Восстановление зависших `queued`/`active` задач подтверждено реальным wall-clock soak-тестом (не только интеграционными тестами против живой БД)

### Content Validation

- [x] **VALID-03**: API отклоняет загрузку, если заявленные размеры изображения (ширина × высота) превышают настраиваемый лимит, до запуска конвертации (защита от decompression bomb)
...
|-------------|-------|--------|
| WEBHOOK-06 | Phase 5 | Done |
| RECON-04 | Phase 6 | Done |
| RECON-05 | Phase 6 | Done |
| VALID-03 | Phase 7 | Done |
