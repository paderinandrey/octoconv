...
- [ ] **WEBHOOK-06**: Оператор может отключить SSRF-блокировку RFC1918 приватных адресов (`WEBHOOK_ALLOW_PRIVATE_IPS`) для деплоев на внутренней сети компании, где `callback_url` легитимно указывает на приватный IP; loopback, link-local (включая cloud metadata endpoint) и unspecified остаются заблокированы всегда, независимо от флага
