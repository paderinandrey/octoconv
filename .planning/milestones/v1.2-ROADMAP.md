...
- [x] **Phase 10: Document Worker & Reconciler Integration** - Separate `cmd/document-worker` binary, `DOCUMENT_ENGINE_TIMEOUT`, and engine-aware reconciler recovery (completed 2026-07-09)
- [x] **Phase 11: API Routing & End-to-End Document Conversion** - `handleCreateJob` routes documents to the document engine and the full pipeline is live-verified across all 6 format pairs (completed 2026-07-09)
...
**Plans**: 4 plans
  - [x] 10-01-PLAN.md — internal/queue document plumbing: `document` queue, `document:convert` task type, per-job unique-lock TTL + no-jitter retry schedule, EnqueueDocumentConvert producer
  - [x] 10-02-PLAN.md — Engine-aware reconciler: Engine field on StaleJob, enqueuer interface extension, engine-routed recovery with fail-closed default
  - [x] 10-03-PLAN.md — HandleDocumentConvert task handler + cmd/document-worker binary consuming the document queue; engine-scoped isDocumentTerminal (timeout = TERMINAL)
  - [x] 10-04-PLAN.md — Dockerfile split: revert Dockerfile.worker to libvips-only, add Dockerfile.document-worker (LibreOffice+fonts+tini), document-worker compose service + .env.example
...
**Plans**: 3 plans + 1 gap closure
  - [x] 11-01-PLAN.md — Engine-aware routing: Converter.Engine()/Registry.EngineFor, widened api.Enqueuer, handleCreateJob document-queue routing + dimension-skip test
  - [x] 11-02-PLAN.md — Live E2E suite (internal/e2e): 6-pair fixtures, HTTP+poll+webhook harness, presigned-download %PDF- checks, docker-compose.e2e.yml override
  - [x] 11-03-PLAN.md — Run the E2E suite against the live docker-compose stack across all 6 pairs + human-verify checkpoint
  - [x] 11-04-PLAN.md — Gap closure (DOC-10 SC#2): extend convert.MIMEType with pdf + 6 document MIME types so document Content-Type matches image-job parity; unit + handler regression tests
...
| 11. API Routing & End-to-End Document Conversion | v1.2 | 4/4 | Complete    | 2026-07-09 |
