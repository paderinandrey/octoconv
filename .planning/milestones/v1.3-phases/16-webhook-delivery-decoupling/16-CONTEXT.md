...
### New binary config
- **D-07:** `cmd/webhook-worker` — тримнутая копия `cmd/worker/main.go`. **Нужно**: Postgres (repo + advisory-lock-соединение + sweeper), Redis (asynq-консюмер `TypeWebhookDeliver`), `WEBHOOK_SIGNING_SECRET`, `WEBHOOK_ALLOW_PRIVATE_IPS` (SSRF-гард доставки Phase 5), sweep/reconciler-env, `WEBHOOK_WORKER_CONCURRENCY`, **и storage (S3/MinIO) + presign-TTL** — ⚠️ КОРРЕКЦИЯ первоначального допущения: `HandleWebhookDeliver` (`internal/worker/worker.go:512-527`) для done-задач генерирует свежий presigned download URL (`h.store.PresignGet(outs[0].ObjectKey, h.presignTTL)`) прямо в webhook-payload (D-09), поэтому webhook-worker'у ОБЯЗАТЕЛЕН storage-клиент + `presignTTL`-конфиг. **НЕ нужно**: convert-registry, engine-таймауты, engine-конвертеры — никакой конвертации в webhook-worker не происходит. Свой `Dockerfile.webhook-worker` (чистый debian-slim, без libvips/LibreOffice/chromium — движковые пакеты не нужны, storage — это Go-клиент minio, не системный пакет).
...
- Postgres-first: job-состояние в Postgres; доставка читает job/outputs оттуда, НО для done-задач ещё пресайнит download URL из S3 → webhook-worker'у нужен storage-клиент (D-07 скорректирован после pattern-mapping)
