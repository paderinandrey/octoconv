...
- **D-11:** [informational] WR-01 (падение api → пустой PromQL-результат читается как 0 при ignoreNullValues=true → даунскейл с живым бэклогом) — НЕ в этой фазе: отдельная работа с трейдоффами семантики триггера, менять поведение прямо перед флагманской приёмкой рискованно. В бэклог/следующий милстоун.
