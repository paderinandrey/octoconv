...
func TestByIP_NotEvadedByForwardedForSpoofing(t *testing.T) {
	srv := NewServer(&fakeRepo{}, &fakeStorage{}, &fakeQueue{}, defaultFakePresetRepo(), defaultFakePresetAdmin(), newFakeResolver(), healthyDeps(), Config{IPRateLimitRPM: 2, MaxUploadBytes: 1 << 20})
	h := srv.Routes()
